import axios from "axios";

export const axiosJwt = axios.create();

//dang nhap
export const loginUser = async (data) => {
  const res = await axios.post("http://localhost:3000/api/user/sign-in", data);
  return res.data;
};

//dang ki
export const sigupUser = async (data) => {
  const res = await axios.post("http://localhost:3000/api/user/sign-up", data);
  return res.data;
};

export const getDetailsUser = async (id, access_token) => {
  const res = await axiosJwt.get(
    `http://localhost:3000/api/user/get-user/${id}`,
    {
      headers: {
        token: `Bearer ${access_token}`,
      },
    }
  );

  return res.data;
};

export const refreshToken = async (access_token) => {
  axios.defaults.headers.common["token"] = `Bearer ${access_token}`;
  const res = await axios.post(
    "http://localhost:3000/api/user/refresh-token",
    {
      withCredentials: true,
    },
    {
      headers: {
        token: `Bearer ${access_token}`,
      },
    }
  );
  return res.data;
};

export const logoutUser = async (id, access_token) => {
  const res = await axiosJwt.post(`http://localhost:3000/api/user/log-out`);
  return res.data;
};

export const updateUser = async (id, data, access_token) => {
  const res = await axiosJwt.put(
    `http://localhost:3000/api/user/update-user/${id}`,
    data,
    {
      headers: {
        token: `Bearer ${access_token}`,
      },
    }
  );

  return res.data;
};

export const deleteUser = async (id, access_token) => {
  const res = await axios.delete(
    `http://localhost:3000/api/user/delete-user/${id}`,

    {
      headers: {
        token: `Bearer ${access_token}`,
      },
    }
  );
  return res.data;
};

export const getAllUser = async (access_token) => {
  const res = await axiosJwt.get(
    `http://localhost:3000/api/user/getAll`,

    {
      headers: {
        token: `Bearer ${access_token}`,
      },
    }
  );
  return res.data;
};

export const senOTP = async (data) => {
  const res = await axios.post(`http://localhost:8080/api/user/senOtp`, data);
  return res.data;
};

export const verifyOtp = async (data) => {
  const res = await axios.post(
    `http://localhost:8080/api/user/verifyOtp`,
    data
  );
  return res.data;
};

export const newPassword = async (data) => {
  const res = await axios.post(
    `http://localhost:8080/api/user/updatePassword`,
    data
  );
  return res.data;
};
